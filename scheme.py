...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("receives a different number of formal parameters and arguments")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        expr = Pair(vals.first.second, vals.second)
...
def do_let_form(vals, env):
    ...
    for expr in vals.first:
        name, value = expr.first, scheme_eval(expr.second.first, env)
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError("the list of formals is not a well-formed list of symbols or if any symbol is repeated")
    for i in formals:
        if not scheme_symbolp(i) or i in formals_lst:
            raise SchemeError("the list of formals is not a well-formed list of symbols or if any symbol is repeated")
        formals_lst.append(i)
