...
(lambda (z) 56 (* z 4) (* z z) 56)
; expect (lambda (z) (begin 56 (* z 4) (* z z) 56))
...
(define (tree-sum-list tree)
  (cond
    ...
    (else (tree-sum-list (entry tree)) 
      (list-sum (entry (entry tree)) (flatten (children (flatten (tree-sum-list (children tree)))))))))
   ...
   ;     ((null? tree) 0)
   ;     ((leaf? tree) tree)
   ;     ((pair? tree) (+ (tree-sum-list (entry tree)) (tree-sum-list (children tree)) ) )
   ;       (else (+ (entry tree) (tree-sum-list (children tree))))))
