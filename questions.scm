...
(define (tree-sums tree)
    ; *** YOUR CODE HERE ***
    (define sum 0)
    (define sub-tree (cons tree nil))
    (tree-sums-helper sub-tree sum))

(define (tree-sums-helper sub-tree sum)
   (if (null? sub-tree)
       nil
       (if (null? (cdar sub-tree))
          (append (cons (+ (caar sub-tree) sum) nil)
                (tree-sums-helper (children sub-tree) sum))
          (append (tree-sums-helper (cdar sub-tree) (+ (caar sub-tree) sum))
                (tree-sums-helper (children sub-tree) sum)))))
